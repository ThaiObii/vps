...
async def regen_ssh_command(interaction: discord.Interaction, container_name: str):
    ...
    if ssh_session_line:
        await interaction.user.send(embed=discord.Embed(
            description=f"🔐 **New SSH Session Command:**\n```{ssh_session_line}```",
            color=0x00ff00
        ))
        await interaction.response.send_message(embed=discord.Embed(
...
async def start_server(interaction: discord.Interaction, container_name: str):
    ...
    try:
        ...
        if ssh_session_line:
            ...
            await interaction.response.send_message(embed=discord.Embed(
                description="⚠️ Instance started, but failed to get SSH session line.",
                color=0xff0000
            ))
    except subprocess.CalledProcessError as e:
        await interaction.response.send_message(embed=discord.Embed(
...
async def restart_server(interaction: discord.Interaction, container_name: str):
    ...
    try:
        ...
        if ssh_session_line:
            ...
            await interaction.response.send_message(embed=discord.Embed(
                description="⚠️ Instance restarted, but failed to get SSH session line.",
                color=0xff0000
            ))
    except subprocess.CalledProcessError as e:
        await interaction.response.send_message(embed=discord.Embed(
...
async def create_server_task(interaction):
    await interaction.response.send_message(embed=discord.Embed(
        description="🔄 **Creating VPS, This might take a few seconds...**",
        color=0x00ff00
    ))
    user = str(interaction.user)
    if count_user_servers(user) >= SERVER_LIMIT:
        await interaction.followup.send(embed=discord.Embed(
            description="🚫 **Error: VPS Limit Reached**",
            color=0xff0000
        ))
        return
    ...
    if ssh_session_line:
        ...
        await interaction.followup.send(embed=discord.Embed(
            description="✅ **VPS created successfully. Check your DMs for details.**",
            color=0x00ff00
        ))
    else:
        await interaction.followup.send(embed=discord.Embed(
            description="⚠️ Something went wrong or the Instance is taking longer than expected. If this problem continues, Contact Support.",
            color=0xff0000
        ))
        subprocess.run(["docker", "kill", container_id])
        subprocess.run(["docker", "rm", container_id])
...
@bot.tree.command(name="start", description="▶️ Starts your instance")
@app_commands.describe(container_name="The name/ssh-command of your Instance")
async def start(interaction: discord.Interaction, container_name: str):
    await start_server(interaction, container_name)


@bot.tree.command(name="stop", description="⏹️ Stops your instance")
@app_commands.describe(container_name="The name/ssh-command of your Instance")
...
async def list_servers(interaction: discord.Interaction):
    ...
    if servers:
        ...
        for server in servers:
            ...
            embed.add_field(name=f"🖥️ {container_name}", value="✅ Active", inline=False)
        await interaction.response.send_message(embed=embed)
    else:
        await interaction.response.send_message(embed=discord.Embed(
...
@bot.tree.command(name="remove", description="🗑️ Removes an Instance")
@app_commands.describe(container_name="The name/ssh-command of your Instance")
async def remove_server(interaction: discord.Interaction, container_name: str):
    ...
    try:
        ...
        await interaction.response.send_message(embed=discord.Embed(
            description=f"🗑️ Instance '{container_name}' removed successfully.",
            color=0x00ff00
        ))
    except subprocess.CalledProcessError as e:
        await interaction.response.send_message(embed=discord.Embed(
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(name="🗑️ `/remove <name>`", value="Removes a server", inline=False)
    embed.add_field(name="▶️ `/start <name>`", value="Start a server", inline=False)
    embed.add_field(name="⏹️ `/stop <name>`", value="Stop a server", inline=False)
    embed.add_field(name="🔐 `/regen-ssh <name>`", value="Regenerates SSH credentials", inline=False)
    embed.add_field(name="🔄 `/restart <name>`", value="Restart a server", inline=False)
